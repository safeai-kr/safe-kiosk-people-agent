...
class BucketMetric:
    ...
    def to_wire(self)->Mapping[str,object]:
        detail = {source.value: {'inside_tick_count': value.inside_tick_count, 'foot_traffic_count': value.foot_traffic_count, 'entry_count': value.entry_count, 'dwell_seconds_sum': float(value.dwell_seconds_sum), 'completed_dwell_session_count': float(value.completed_dwell_session_count), 'timeout_closed_count': value.timeout_closed_count, 'unconfirmed_entry_count': value.unconfirmed_entry_count, 'interrupted_session_count': value.interrupted_session_count} for source, value in self.source_detail.items()}
        return {'bucket_start':self.bucket_start.isoformat(),'bucket_end':self.bucket_end.isoformat(),'estimated_people_count':self.estimated_people_count,'peak_people_count':self.peak_people_count,'foot_traffic_count':self.foot_traffic_count,'entry_count':self.entry_count,'dwell_seconds_sum':float(self.dwell_seconds_sum),'completed_dwell_session_count':float(self.completed_dwell_session_count),'wifi_observation_count':self.wifi_observation_count,'ble_observation_count':self.ble_observation_count,'confidence_score':float(self.confidence_score),'quality_flags':sorted(set(self.quality_flags)),'source_detail':detail,'threshold_version':self.threshold_version,'metric_version':self.metric_version,'revision':self.revision,'generated_at':self.generated_at.isoformat()}
@dataclass(frozen=True)
...
class SensorStatus:
    ...
    def to_wire(self)->Mapping[str,object]: return {'observed_at':self.observed_at.isoformat(),'kismet_status':self.kismet_status.value,'wifi_status':self.wifi_status.value,'ble_status':self.ble_status.value,'metrics_status':self.metrics_status.value,'upload_status':self.upload_status.value,'last_wifi_observed_at':self.last_wifi_observed_at.isoformat() if self.last_wifi_observed_at else None,'last_ble_observed_at':self.last_ble_observed_at.isoformat() if self.last_ble_observed_at else None,'last_bucket_created_at':self.last_bucket_created_at.isoformat() if self.last_bucket_created_at else None,'last_upload_at':self.last_upload_at.isoformat() if self.last_upload_at else None,'recovery_counts':dict(self.recovery_counts),'cpu_percent':float(self.cpu_percent),'memory_bytes':self.memory_bytes,'disk_free_bytes':self.disk_free_bytes,'pending_outbox_count':self.pending_outbox_count,'last_error':None if self.last_error is None else {'component':self.last_error.component,'code':self.last_error.code,'message':self.last_error.message,'occurred_at':self.last_error.occurred_at.isoformat()},'threshold_version':self.threshold_version,'metric_version':self.metric_version}
