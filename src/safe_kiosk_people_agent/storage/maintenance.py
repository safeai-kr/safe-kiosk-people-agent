...
class StorageMaintainer:
    ...
    def run_once(self,now:datetime,free_bytes:int|None=None,state_bytes:int=0)->MaintenanceResult:
        ...
        if pressured:
            pause = self.control.request_pause(reason='storage_full',boot_id='unknown',now=now)
            if not self.control.wait_for_ack(generation=pause.generation, required={'kismet','wifi','ble'}, timeout_seconds=20):
                return MaintenanceResult((),False,'storage_full',{},False)
